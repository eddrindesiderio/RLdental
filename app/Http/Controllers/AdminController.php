    ...
    public function editAdminProfile(Request $request)
{
    ...
    $request->validate([
        ...
        'number' => 'required|regex:/^09\d{9}$/',  // Validate phone number format (starts with 09 and 11 digits)
        'address' => 'required|string|max:255',
        'dob' => 'required|date|before:today',  // Ensure date of birth is valid
    ]);
...
public function adminChangePassword(Request $request)
{
    ...
    $request->validate([
        ...
        'new_password' => [
            'required',
            'regex:/^(?=.*[a-zA-Z])(?=.*\d).{6,}$/',  // Password must be at least 6 characters and contain both letters and numbers
        ],
        'confirm_password' => 'required|same:new_password',  // Confirm new password must match the new password
    ]);
