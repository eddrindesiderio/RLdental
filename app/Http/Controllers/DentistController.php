...
class DentistController extends Controller
{
    ...
    {
        // Validate the parameters with custom rules
        $request->validate([
            'full_name' => ['required', 'regex:/\b(\w+)\s+(\w+)\b/', 'max:100'],  // Custom validation for first and last name
        ...
        // Ensure the full name contains at least two words (first and last name) and doesn't exceed 8 words
        $fullName = $request->input('full_name');
        $words = explode(' ', $fullName);
        
        if (count($words) < 2 || count($words) > 8) {
            // Add the error message to the session and redirect back with previous input
            return redirect()->back()->withInput()->withErrors(['full_name' => 'Full Name must contain at least a first name and a last name, and cannot exceed 8 words.']);
        }
